...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = """
            SELECT p.id AS id,
                   ...
                   p.status AS status,
                   (
                     COALESCE(ts_rank(to_tsvector('english', coalesce(p.name,'') || ' ' || coalesce(p.sku,'') || ' ' || coalesce(p.description,'')),
                                       plainto_tsquery('english', :q)), 0)
                     + CASE WHEN p.name ILIKE ('%' || :q || '%') THEN 1.0 ELSE 0 END
                     + CASE WHEN p.sku ILIKE ('%' || :q || '%') THEN 1.0 ELSE 0 END
                     + CASE WHEN p.description ILIKE ('%' || :q || '%') THEN 1.0 ELSE 0 END
                   ) AS rank,
